...
router.post('/login/admin-service-provider', async (req, res) => {
  ...
  try {
    ...
    // Check if user has a password (Admin and Service Provider should have passwords)
    if (!user.password) {
...
// Service Provider Registration (public, requires admin approval)
router.post('/signup/service-provider', async (req, res) => {
  const { name, email, password, phone, businessName, address, servicesProvided } = req.body;
  ...
  try {
    ...
    // Create new service provider user with pending approval
      ...
      data: {
        ...
        serviceProvider: {
          create: {
            ...
            phone: phone.trim(),
            address: address || '',
            serviceType: servicesProvided || 'Crop Insurance',
            status: 'pending',
          },
        },
      },
    ...
    Logger.error('Service Provider signup error', { error });
    if (error.code === 'P2002') {
...
// Public signup is available for Farmers (OTP) and Service Providers (with approval)

// Signup for Farmer (creates account and sends OTP)
router.post('/signup/farmer', async (req, res) => {
  const { name, mobileNumber, aadhaarNumber, address, village, district, consentGranted } = req.body;
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Record consent (will be done after transaction to avoid type issues)
      // Consent recording moved outside transaction
