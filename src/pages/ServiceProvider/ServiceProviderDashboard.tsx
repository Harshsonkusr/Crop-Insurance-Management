...
import ServiceProviderDashboardFooter from '@/components/Footers/ServiceProviderDashboardFooter';
...
const ServiceProviderDashboard = () => {
  ...
  return (
    <div className="flex h-screen bg-sp-off-white">
      {/* Sidebar */}
      <aside className={`hidden md:flex flex-col bg-sp-primary-DEFAULT shadow-md transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
        {/* Logo/Brand */}
        ...
        <nav className="flex-1 mt-4 overflow-y-auto px-2">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className={`flex items-center py-2.5 px-4 rounded-lg transition duration-200 text-white hover:bg-sp-primary-dark transform hover:-translate-y-1 hover:scale-105 mb-2 ${
                isActive(link.path) ? 'bg-sp-primary-dark' : ''
              }`}
            >
              {link.icon}
          ...
          <div className="p-4 border-t border-white/20">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white font-medium truncate">{user?.name || 'Service Provider'}</p>
                <p className="text-white/70 text-xs truncate">Service Provider</p>
              </div>
            </div>
            <button
      ...
      <div className="flex-1 flex flex-col animate-fade-in overflow-hidden">
                ...
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel>
                    <div className="flex flex-col space-y-1">
                      <p className="text-sm font-medium">{user?.name || 'Service Provider'}</p>
                      <p className="text-xs text-gray-500">Service Provider</p>
                    </div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem asChild>
                    <Link to="/service-provider-dashboard/settings" className="flex items-center gap-2">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden fixed inset-0 z-40 bg-black/50" onClick={() => setIsMobileMenuOpen(false)}>
            <div className="absolute right-0 top-0 h-full w-64 bg-sp-primary-DEFAULT shadow-xl">
              <div className="p-4 border-b border-white/20">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className="text-white font-medium">{user?.name || 'Service Provider'}</p>
              ...
              <nav className="p-4 space-y-2">
                {navLinks.map((link) => (
                  <Link
                    key={link.name}
                    to={link.path}
                    className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-white ${
                      isActive(link.path)
                        ? 'bg-white/20'
                        : 'hover:bg-white/10'
                    }`}
        ...
        {/* Footer */}
        <ServiceProviderDashboardFooter />
      </div>
      ...
      <AlertDialog open={isLogoutDialogOpen} onOpenChange={setIsLogoutDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to log out? You will need to sign in again to access the service provider dashboard.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
