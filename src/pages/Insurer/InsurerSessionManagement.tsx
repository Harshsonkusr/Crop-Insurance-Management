...
const InsurerSessionManagement: React.FC = () => {
  ...
  const fetchSessions = async () => {
    try {
      ...
      logger.insurer.system('Fetched active sessions', { count: response.data?.length || 0 });
    } catch (err: any) {
      logger.insurer.error('Error fetching sessions', { error: err });
  ...
  const confirmRevokeSession = async () => {
    ...
    try {
      ...
      logger.insurer.system('Session revoked', { sessionId: selectedSessionId });
      setRevokeDialogOpen(false);
      fetchSessions();
    } catch (err: any) {
      logger.insurer.error('Error revoking session', { error: err });
  ...
  const confirmRevokeAll = async () => {
    try {
      ...
      logger.insurer.system('All sessions revoked');
      setRevokeAllDialogOpen(false);
      setTimeout(() => {
        window.location.href = '/login';
      }, 1000);
    } catch (err: any) {
      logger.insurer.error('Error revoking all sessions', { error: err });
...
export default InsurerSessionManagement;
