...
const InsurerClaimManagement = () => {
  ...
  const handleDeleteClaim = async (claimId: string) => {
    ...
    try {
      await api.delete(`/insurer/claims/${claimId}`);
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 flex items-center gap-3 tracking-tight">
            <span className="p-2 bg-purple-100 rounded-xl text-purple-700 shadow-inner">
              <FileText className="h-8 w-8" />
            </span>
            Claim Inventory
          </h1>
          <p className="text-gray-600 mt-2 ml-14 font-medium italic underline underline-offset-4 decoration-purple-200">Management and verification protocol for assigned insurance nodes.</p>
        </div>
      </div>
      ...
      {filteredClaims.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredClaims.map((claim) => (
            <Card key={claim._id || claim.id} className="transition-all hover:shadow-xl hover:-translate-y-1 border-none shadow-md overflow-hidden bg-white/50 backdrop-blur-sm group border-l-4 border-l-transparent hover:border-l-purple-500">
              <CardContent className="p-6">
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-3 flex-wrap">
                      <h3 className="text-lg font-extrabold text-gray-900 group-hover:text-purple-700 transition-colors tracking-tight">
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Link to={`/insurer-dashboard/claims/${claim._id || claim.id}`}>
                      <Button variant="ghost" size="sm" className="h-9 w-9 p-0 text-blue-600 hover:bg-blue-50 rounded-xl" title="View Details">
                        <Eye className="h-4 w-4" />
                      </Button>
                    </Link>
                    <Link to={`/insurer-dashboard/claims/${claim._id || claim.id}/edit`}>
                      <Button variant="ghost" size="sm" className="h-9 w-9 p-0 text-purple-600 hover:bg-purple-50 rounded-xl" title="Edit Claim">
...
export default InsurerClaimManagement;
