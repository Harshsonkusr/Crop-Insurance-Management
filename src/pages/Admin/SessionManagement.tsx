...
const SessionManagement: React.FC = () => {
  ...
  const fetchSessions = async () => {
    try {
      ...
      logger.admin.system('Fetched active sessions', { count: response.data?.length || 0 });
    } catch (err: any) {
      logger.admin.error('Error fetching sessions', { error: err });
  ...
  const confirmRevokeSession = async () => {
    ...
    try {
      ...
      logger.admin.system('Session revoked', { sessionId: selectedSessionId });
      setRevokeDialogOpen(false);
      fetchSessions();
    } catch (err: any) {
      logger.admin.error('Error revoking session', { error: err });
  ...
  const confirmRevokeAll = async () => {
    try {
      ...
      logger.admin.system('All sessions revoked');
      setRevokeAllDialogOpen(false);
      setTimeout(() => {
        window.location.href = '/login';
      }, 1000);
    } catch (err: any) {
      logger.admin.error('Error revoking all sessions', { error: err });
