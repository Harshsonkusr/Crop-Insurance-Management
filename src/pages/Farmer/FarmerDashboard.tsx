        ...
        <nav className="mt-4">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
               className={`flex items-center py-2.5 px-4 rounded-lg transition duration-200 text-white hover:bg-green-900 transform hover:-translate-y-1 hover:scale-105 ${location.pathname === link.path ? 'bg-green-700' : ''}`}
            >
              {link.icon}
